...
class DetectPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.os_card = StatusCard("OS", "Not checked")
        self.conda_card = StatusCard("Conda", "Not checked")
        self.gpu_card = StatusCard("GPU", "Not checked")
        self.disk_card = StatusCard("Disk", "Not checked")
    ...
    def set_snapshot(self, snapshot: EnvSnapshot) -> None:
        self.os_card.set_status(snapshot.os, "Supported" if snapshot.is_windows_supported else "Unsupported")
        conda_value = snapshot.conda.version or "Not found"
        conda_detail = snapshot.conda.path or "Conda will be installed later"
        ...
        if snapshot.gpu:
            ...
            self.gpu_card.set_status("CPU mode", "No NVIDIA GPU detected")
        self.disk_card.set_status(f"{snapshot.free_disk_gb:.1f} GB free", "User workspace")
